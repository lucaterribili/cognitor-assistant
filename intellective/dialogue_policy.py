...
DIALOGUE_POLICY_EMBED_DIM: int = 32
DIALOGUE_POLICY_HIDDEN_DIM: int = 64
...
class DialoguePolicy(nn.Module):
    ...
    Architettura:
    - Embedding degli intent (storia conversazione + intent corrente)
    ...
    def __init__(
        ...
        Args:
            ...
            embed_dim:    Dimensione degli embedding degli intent.
        ...
        # Embedding condiviso per intent nel contesto e intent corrente (pad_idx=0)
    ...
    def forward(
        ...
        """
        # Embedding del contesto storico
        context_emb = self.intent_embedding(context_intents)   # [B, T, E]
    ...
    def predict(
        ...
        Args:
            context_intents: [1, T] — contesto storico (batch size 1)
        ...
        self.eval()
        with torch.no_grad():
            logits = self.forward(context_intents, current_intent)
