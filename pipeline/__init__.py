...
from pipeline.merge_data import merge_training_data
...
def run_full_pipeline(
    ...
    if merge_data:
        ...
        merge_training_data(data_path)
    else:
        print("\n[1/4] Merge dei dati - SKIPPED")

    print("\n[2/4] Generazione dati NLU...")
    build_intents(data_path=data_path)

    if train_fasttext:
        print("\n[3/4] Training FastText...")
        train_embedder()

    else:
        print("\n[3/4] Training FastText - SKIPPED")
...
__all__ = ['run_full_pipeline', 'build_intents', 'merge_training_data']
